...
def generator(z, batch_size=batch_size, phase_train=True, reuse=False):
    ...
    with tf.variable_scope("gen", reuse=reuse):
        ...
        g_6 = tf.nn.conv3d_transpose(g_5,
                                     weights['wg6'],
...
def discriminator(inputs, phase_train=True, reuse=False):
    """Discriminate between true and generated."""
    strides = [1, 2, 2, 2, 1]

    with tf.variable_scope("dis", reuse=reuse):
        d_1 = tf.nn.conv3d(inputs,
                           weights['wd1'],
                           strides=strides,
                           padding="SAME")
